...
from torch_geometric.nn import GATv2Conv, GatedGraphConv, JumpingKnowledge, global_max_pool, global_mean_pool, \
    global_add_pool, global_sort_pool, GlobalAttention, GraphMultisetTransformer, SumAggregation, MeanAggregation, \
    MaxAggregation, MinAggregation, PowerMeanAggregation, SoftmaxAggregation, AttentionalAggregation, \
    EquilibriumAggregation, EGConv, MultiAggregation
...
class EGC(torch.nn.Module):
    def __init__(self, passes, inputLayerSize, outputLayerSize, pool, aggregators=["symnorm"], shouldJump=True,
        ...
        self.egcs = nn.ModuleList([EGConv(in_channels=inputLayerSize if i == 0 else self.modSize,
                                          out_channels=self.modSize, aggregators=aggregators, num_heads=num_heads,
                                          num_bases=num_bases) for i in range(passes)])
...
class GAT(torch.nn.Module):
    def __init__(self, passes, inputLayerSize, outputLayerSize, numAttentionLayers, mode, pool, k, dropout,
        ...
        if pool == "add":
            ...
            self.pool = GraphMultisetTransformer(in_channels=fcInputLayerSize - 1, hidden_channels=fcInputLayerSize - 1,
                                                 out_channels=fcInputLayerSize - 1, num_nodes=1400, num_heads=5,
    ...
    def forward(self, x, edge_index, edge_attr, problemType, batch):
        ...
        x = torch.cat(
            (x.reshape(1, x.size(0) * x.size(1)), problemType.unsqueeze(1))
            , dim=1
        )
...
class GAT_SAVE(torch.nn.Module):
    def __init__(self, passes, inputLayerSize, outputLayerSize, numAttentionLayers, mode, pool, k, dropout,
        ...
        self.gats = nn.ModuleList([GATv2Conv(inputLayerSize, inputLayerSize, heads=numAttentionLayers, concat=False,
                                             dropout=0.0, edge_dim=1).jittable() for i in range(passes)])
        if self.passes and self.shouldJump:
            self.jump = JumpingKnowledge(self.mode, channels=inputLayerSize, num_layers=self.passes)
        if self.mode == 'cat' and self.shouldJump:
            self.jump = JumpingKnowledge(self.mode, channels=inputLayerSize, num_layers=self.passes)
